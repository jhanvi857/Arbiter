...
import { signupUser, resendVerification } from '@/lib/arbiter-api'
        ...
        {!isSuccess ? (
          <Card className="border border-border bg-card/70 backdrop-blur-md shadow-2xl rounded-2xl">
            <CardHeader className="text-center pb-2">
              ...
              <CardTitle className="text-3xl font-bold text-foreground tracking-tight">Get Started</CardTitle>
              <CardDescription className="text-foreground/75 mt-1">
                Create an account and start optimizing queries with Arbiter
              </CardDescription>
            </CardHeader>
          ...
          <Card className="border border-border bg-card/70 backdrop-blur-md shadow-2xl rounded-2xl">
            <CardHeader className="text-center pb-2">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/20 text-primary">
                <MailOpen className="w-8 h-8 animate-pulse text-primary" />
              </div>
              <CardTitle className="text-3xl font-bold text-foreground tracking-tight">Verify Your Email</CardTitle>
              <CardDescription className="text-foreground/75 mt-2">
                We've sent a verification link to <span className="font-semibold text-primary">{registeredEmail}</span>.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4 pt-4 text-center">
              ...
              <div className="rounded-lg bg-primary/10 border border-primary/20 px-4 py-3 text-xs text-primary font-medium">
                A verification link has been successfully sent.
              </div>
            </CardContent>
            <CardFooter className="flex flex-col gap-3 justify-center pb-6">
              <Link href="/login" className="w-full">
                <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90 py-6 text-base font-semibold transition-all shadow-md shadow-primary/10 cursor-pointer">
                  Go to Log In
                </Button>
              </Link>
              <button 
                onClick={async () => {
                  try {
                    ...
                    alert(res.message || "Verification link resent successfully!")
                  } catch (err) {
              ...
              {error && (
                <div className="rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2 text-xs text-red-500 text-center w-full">
                  {error}
                </div>
              )}
            </CardFooter>
